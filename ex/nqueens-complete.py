...
class Interpretation():
	...
	def __init__(self, N):
		...
		"""
		self.num_vars = N
		self.vars = [-1] * self.num_vars

	def safe_place(self, n, row):
		""" Detects if a position in the board is a safe place for a queen"""
		for num_queen in xrange(0 , n): # Iterate over the columns on the left of n
...
class Solver():
	...
	def solve(self):
		"""
		Implements an iterative algorithm to solve the instance of a problem
		"""
		curr_sol = Interpretation(self.problem) # Empty interpretation
		n = 0
		while n >= 0: # While we are below root node
			# Assignment previous to testing safe place
			next_row = curr_sol.vars[n] + 1 # Next branch
			if next_row == self.problem: # If no more branches, Backtracking
				curr_sol.vars[n] = -1 # Restart node
				n = n - 1 # Go to previous node
				continue
			curr_sol.vars[n] = next_row # Assign new branch to node
			if curr_sol.safe_place(n, next_row): # Check if it is correct
				n = n + 1 # Extend branch
				if n == self.problem: # Complete assignment
					return curr_sol
			# Assignment after testing safe place
			# nr = curr_sol.vars[n] + 1 # Next branch
			# while nr < self.problem:
			# 	if curr_sol.safe_place(n, nr):
			# 		curr_sol.vars[n] = nr
			# 		n = n + 1 # Extend branch
			# 		if n == self.problem: # Complete assignment
			# 			return curr_sol
			...
			# 	curr_sol.vars[n] = -1 # Restart node
			# 	n = n - 1 # Go to previous node
		return None # Never will happen as for N >= 4 there is a solution
